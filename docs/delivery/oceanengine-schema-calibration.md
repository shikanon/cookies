...
| `charging_mode`                    | oCPM/oCPC/CPC/CPM                          | `derived_enum`       | 随优化目标和投放模式                                                     | 展示量解析为 CPM，点击量解析为 CPC，转化目标解析为 oCPM；该字段由平台条件决定，不是自由输入                                                                                    | `observed` / `sample_only`                       |
| `bid`                              | 项目出价/深度出价                          | `money`              | 稳定成本 + 支持出价的目标/深度方式                                       | CPM 为 `4～100` 元；标准 CPC 为 `0.1～100` 元；oCPM/oCPC 为 `0.01～10000` 元且不超过日预算。Runner Prepare 回读页面范围；不一致时按页面漂移停止                              | `observed`                                       |
...
本地未提交校验已取得：日预算 `300～9999999.99` 元；项目出价按计费方式使用独立范围；ROI 系数为 `0.01～100`；项目名称为 1～50 个字。范围规则见 `internal/platform/oceanengineconstraints/oceanengine-bid-constraints-v1.json`。它们可以进入黄金路径预检，但仍需保留平台版本和条件路径；不得外推到其他营销目的、载体或平台。
